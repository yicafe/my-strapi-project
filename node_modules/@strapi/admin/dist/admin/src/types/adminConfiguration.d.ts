 ...
 */
export interface CMAdminConfiguration extends Omit<Configuration, 'layouts'>, Omit<Struct.ContentTypeSchema, 'uid' | 'collectionName' | 'globalId' | 'modelName'> {
    apiID: string;
    isDisplayed: boolean;
    layouts: {
        list: null;
...
export type NonRelationLayout = Layouts['edit'][number][number] & {
    fieldSchema: Pick<Exclude<Schema.Attribute.AnyAttribute, {
        type: 'relation';
    }>, 'pluginOptions' | 'type'>;
    /**
...
export interface RelationLayout extends Omit<NonRelationLayout, 'fieldSchema'> {
    fieldSchema: Pick<Extract<Schema.Attribute.AnyAttribute, {
        type: 'relation';
